...
static void *circular_buffer_task_tx( void *_URLContext)
{
    ...
    for(;;) {
        ...
        while (len) {
            ...
            av_assert0(len > 0);
            if (!s->is_connected) {
                printf("send rtp msg circular_buffer_task_tx\n");
...
static int udp_open(URLContext *h, const char *uri, int flags)
{
    ...
    if (s->is_multicast && !(h->flags & AVIO_FLAG_WRITE)) {
        ...
        printf("chuang jian read dest_addr socket\n");
    }
    ...
    {
        printf("chuang jian read my_addr socket\n");
    }
